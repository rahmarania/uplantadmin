...
if(isset($_POST['submit'])){
    ...
    if(empty($err)){
        ...
        for($i=1;$i<count ($sheetData);$i++){
            $id = $sheetData[$i]['0'];
            $namapengguna = $sheetData[$i]['1'];
            $email = $sheetData[$i]['2'];
            $pekerjaan = $sheetData[$i]['3'];
            $telepon = $sheetData[$i]['4'];

            $sql1 = "insert into pengguna(namapengguna, email, pekerjaan, telepon) value('$namapengguna', '$email', '$pekerjaan', '$telepon')";
